...
public class SimHash {
	...
	/**
	 * In this method, use int(32 bits) to store hashcode. use 1 as all words
	 * weight for simple reason. use Hamming distance as hashcode distance.
	 * 
	 * @author arthur503
	 */
	public SimHash() {
	...
	public int compareString(String str1, String str2) {
		//System.out.println("SimHash compare string of: \"" + str1 + "\" AND \""
		//		+ str2 + "\"");
		int hash1 = getHashCode(str1);
		int hash2 = getHashCode(str2);

		int distance = getDistance(hash1, hash2);
		//System.out.println("SimHash string distance of: \"" + str1
		//		+ "\" AND \"" + str2 + "\" is:" + distance);
		return distance;
	}

	/**
	 * Use hamming distance in this method. Can change to other distance like
	 * Euclid distance or p-distance, etc.
	 * 
	 * @param hash1
	 * @param hash2
	 * @return
	...
	private int getHashCode(String str) {
		...
		for (int i = 0; i < array.length; i++) {
			...
			//System.out.println("String \"" + array[i] + "\"" + " hashcode is:"
			//		+ Integer.toBinaryString(hash));
			for (int j = HASH_LENGTH - 1; j >= 0; j--) {
				int bit = (hash >> j) & 1;
				// System.out.println("shift j is:"+j+" bit is:"+bit);

				// Different keyword may have different weight. add or minus
				// their weight here.
				// For simple reason, all weight are assigned as 1 in this
				// method.
				if (bit == 1) {
			...
			/*
			 * System.out.println("hashbits is:"); for(int
			 * k=0;k<HASH_LENGTH;k++){
	...
	public static void main(String[] argv) {
		String str1 = "SimHash an art";
		String str2 = "SimHash the art";

		SimHash sim = new SimHash();
		System.out.println(sim.compareString(str1, str2));
